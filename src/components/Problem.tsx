import { Banknote, Clock, BatteryLow } from "lucide-react";
        ...
        <section id="features" className="section" style={{ background: 'var(--bg-secondary)', position: 'relative', overflow: 'hidden' }}>
            <div className="container">
                <div className="reveal" style={{ textAlign: 'center', marginBottom: '3rem' }}>
                    ...
                    <h2 style={{ fontSize: '2.8rem', fontWeight: 900, marginBottom: '1rem', color: 'var(--text-primary)' }}>
                    ...
                    <p style={{ maxWidth: '800px', margin: '0 auto', fontSize: '1.1rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                        You spend thousands on lead gen, only to let high-intent buyers slip through your fingers.
                    </p>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2.5rem' }}>
                    {problems.map((p, i) => (
                        <div key={i} className={`glass reveal delay-${(i + 1) * 100}`} style={{
                            padding: '2.5rem',
